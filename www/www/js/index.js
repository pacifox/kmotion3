...
KM.config = {
    ...
    ptz_current_y:   0,
    sched:           1, // the current schedule
    sched_pastebin: '',
    error_str:      '', // the error string
    error_search_str: /failed|error/i,
    track_coded_str: ''  // temp for coded str
...
KM.feed_cache = function (feed) {
	...
	get_jpeg: function (callback, session_id) {
		...
		if (KM.www_rc.low_cpu) {
		    ...
		    var target_ms = (KM.browser.browser_SP)?(1000):(200);
...
KM.conf_config_track_send = function() {  

    // A function that sends the coded string generated by 'conf_config_track()'
    // via a xmlHttp request back to the server, this is called by a 
    // 'add_timeout_id()' call instead of being in line to avoid rapid 
    // selections killing this transmission.
    //
    // expects:
    //
    // returns:
    ...
    var coded_str = KM.config.track_coded_str;
	...
	KM.kill_timeout_ids(KM.CONFIG_SEND);
    }
};
...
KM.conf_config_track = function() {
	...
	sync: function() {
	    ...
	    KM.config.track_coded_str = coded_str;
	    KM.add_timeout_id(KM.CONFIG_SEND, setTimeout(function () {KM.conf_config_track_send(); }, 1));
...
KM.conf_feed_html = function () {
    ...
    '<div style="float:left; width:100px;" class="config_text_margin disabled" id="feed_text_5">' +
        'Input&nbsp;:<br>' +
        '<div style="margin-top:6px;"></div>' +
        'Proxy&nbsp;:<br>' +
        '<div style="margin-top:6px;"></div>' +
    ...
    '<div class="config_tick_margin disabled" id="feed_text_11">' +	
    ...
    '<div class="config_text_margin disabled" style="width:412px;" id="feed_text_12">' +
        'at : ' +
    ...
    '<div class="config_text_margin disabled" id="feed_text_13">' +	
        'Schedule : '+ 
	'<select id="feed_sched_motion" onchange="KM.conf_feed_highlight_apply();" disabled>' +
    ...
    '<div class="config_text_margin disabled" style="width:412px;" id="feed_text_15">' +
        'of : ' +
        '<input type="text" id="feed_snap" size="4" onfocus="KM.conf_feed_highlight_apply();" disabled>' + 
        ' Seconds, (300 Seconds recommended)' +
    '</div>' +

    '<div class="config_text_margin disabled" id="feed_text_16">' +	
        'Schedule : '+ 
	'<select id="feed_sched_snap" onchange="KM.conf_feed_highlight_apply();" disabled>' +
	    '<option value="0">None</option>' +
	'</select>' +
    '</div>' +
    ...
    '<div class="config_text_margin disabled" id="feed_text_20">' +	
        'Schedule : '+ 
	'<select id="feed_sched_email" onchange="KM.conf_feed_highlight_apply();" disabled>' +
	    '<option value="0">None</option>' +
	'</select>' +
     '</div>' +
    ...
    '</div>' +
     ...
     '<div class="config_tick_margin disabled" style="width:459px;" id="feed_text_21">' +	
        '<input type="checkbox" id="feed_audio_enabled" onclick="KM.conf_feed_highlight_apply();" disabled>' +
        'Enable audible file to be played on motion detection.' +
    '</div>' +
    
     '<div class="config_text_margin disabled" id="feed_text_22">' +	       
        'File : <input type="text" id="feed_audio_file" size="40" onfocus="KM.conf_feed_highlight_apply();" disabled>' +
    '</div>' +
    
     '<div class="config_text_margin disabled" style="width:412px;" id="feed_text_23">' +
...
KM.conf_feed_highlight_apply = function () {
    ...
    document.getElementById('feed_text_25').style.color = KM.BLUE;
};
...
KM.conf_schedule_html = function () {
    ...
    // A function that generates the schedule HTML. It create the 
    // schedule config screen on the config backdrop 'slab'. __NOTE__ There 
    // is a seperate function to generate the schedule exception HTML.
    //
    // expects:
    //
    // returns:
   ...
   '</div><br>' +
    ...
    KM.conf_sched_weekday(KM.config.sched, 1, 'Monday') +
    KM.conf_sched_weekday(KM.config.sched, 2, 'Tuesday') +
    KM.conf_sched_weekday(KM.config.sched, 3, 'Wednesday') +
    KM.conf_sched_weekday(KM.config.sched, 4, 'Thursday') +
    KM.conf_sched_weekday(KM.config.sched, 5, 'Friday') +
    KM.conf_sched_weekday(KM.config.sched, 6, 'Saturday') +
    KM.conf_sched_weekday(KM.config.sched, 7, 'Sunday') +

    '<div class="config_text_margin">' +
	'<input type="button" id="sched_apply" disabled="disabled" OnClick="KM.conf_sched_apply();" value="Apply Schedule Changes"> ' +
    '</div>';
    
    document.getElementById('config_html').innerHTML = html_str;
    document.getElementById('sched_select').selectedIndex = KM.config.sched - 1;  
};
...
KM.conf_sched_change = function() {
    ...
    // A function that changes the selected schedule
    //
    // expects:
    //
    // returns: 
    ...
    KM.config.sched = document.getElementById('sched_select').selectedIndex + 1;
    KM.add_timeout_id(KM.MISC_JUMP, setTimeout(function () {KM.conf_schedule_html(); }, 1));
};
...
KM.conf_sched_apply = function () {

    // A function that applys the schedule changes
    //
    // expects:
    //
    // returns:
	...
	KM.www_rc['sched_tline' + line][KM.config.sched] = encoded;
    }
    
    KM.conf_config_track.sched_modified(KM.config.sched);
    KM.conf_config_track.sync();
